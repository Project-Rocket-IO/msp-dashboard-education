...
(xhttp.onload = function () {
  ...
  document
    .getElementById("company-logo-input-create")
    .addEventListener("change", function (event) {
      var imgElement = document.getElementById("company-logo-img-create");
...
companies.forEach((company) => {
  document
    .getElementById(`company-logo-input-edit-${company}`)
    .addEventListener("change", function (event) {
      var imgElement = document.getElementById(
        `company-logo-img-edit-${company}`
      );
      var file = this.files[0];
      var reader = new FileReader();
...
// Convert forms NodeList to an array and iterate over each form
Array.prototype.slice.call(forms).forEach(function (form) {
  form.addEventListener(
    "submit",
    function (e) {
      ...
      if (editlist) {
        if (fields.every((field) => field.value !== "") && editlist) {
          const t = companyList.get({ id: idField.value });
          Array.from(t).forEach(function (item) {
            const isid = new DOMParser().parseFromString(
              item._values.id,
              "text/html"
            ).body.firstElementChild.innerHTML;
...
document
  ...
  .addEventListener("click", function () {
    ...
    if (activePage && activePage.previousSibling) {
